...
describe('netlify-specific', () => {
  ...
  it('removes conflicting SPF record before creating', async () => {
    fake.seedZone(DOMAIN, ZONE_ID)
    fake.seedRecord(ZONE_ID, { id: 'spf-old', type: 'TXT', hostname: DOMAIN, value: 'v=spf1 include:old.example.com ~all' })
  ...
  it('does not remove unrelated TXT records', async () => {
    fake.seedZone(DOMAIN, ZONE_ID)
    fake.seedRecord(ZONE_ID, { id: 'unrelated', type: 'TXT', hostname: DOMAIN, value: 'some-other-verification=abc123' })
